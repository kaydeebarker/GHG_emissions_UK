...
#Assign headers/column names for UK sectors dataframe
...
GHG_UK22 <- read_ods(
  ...
  rename("TES_Sector" = "TES Sector") %>% #rename variables for easier coding
  rename("TES_Subsector" = "TES Subsector") %>%
  rename("IPCC_code" = "IPCC Code") %>%
  rename("GHG_group" = "GHG Grouped") %>%
  rename("Emissions" = "Emissions (MtCO2e)")
...
GHG_regions$IPCC_name <- gsub('_', ' ', GHG_regions$IPCC_name) #replace underscores with spaces to make nicer to read
...
GHG_regions <- GHG_regions %>%
  mutate(subcode = case_when(grepl(paste(Bioenergy, collapse='|'), SourceName, ignore.case = TRUE) ~ "a", #for LULUCF
                             grepl(paste(Cropland, collapse='|'), SourceName, ignore.case = TRUE) ~ "b",
                             grepl(paste(Forestry, collapse='|'), SourceName, ignore.case = TRUE) ~ "c",
                             grepl(paste(Grassland, collapse='|'), SourceName, ignore.case = TRUE) ~ "d",
                             grepl(paste(Peatland, collapse='|'), SourceName, ignore.case = TRUE) ~ "e",
                             grepl(paste(Settlement, collapse='|'), SourceName, ignore.case = TRUE) ~ "f",
                             grepl(paste(O_LULUCF, collapse='|'), SourceName, ignore.case = TRUE) ~ "g", 
                              .default = "")) %>%
  unite('IPCC_code2', c(IPCC_code, subcode), remove = FALSE, sep = "_") #combine IPCC code with subcode
...
GHG_simpUK <- GHG_UK22 %>%
  filter(Year == "2022") %>%
  group_by(TES_Sector, TES_Subsector, Source, Activity, GHG, GHG_group) %>%
  summarize(Temissions = sum(Emissions, na.rm = TRUE))

#Simplify regional dataset
GHG_simp_regions <- GHG_regions2 %>%
  filter(EmissionYear == "2021") %>%
  group_by(TES_Sector, TES_Subsector, RegionName, SourceName, IPCC_name, GHG) %>%
...
links <- data.frame(source=c(paste0(GHG_simpUK$TES_Subsector), paste0(GHG_simpUK$TES_Sector)), 
                    target=c(paste0(GHG_simpUK$TES_Sector), paste0(GHG_simpUK$GHG)),
...
links2 <- data.frame(source=c(paste0(GHG_simpUK$GHG_group), paste0(GHG_simpUK$TES_Sector)), 
...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Bar graph showing greenhouse gas emissions in kilotonnes of CO2 equivalents in the bioenergy crops subsector in 2021 by source, colored by greenhouse gas identity."}
...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Bar graph showing greenhouse gas emissions in kilotonnes of CO2 equivalents in the cropland mineral soils change subsector in 2021 by source, colored by greenhouse gas identity."}
...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Bar graph showing greenhouse gas emissions in kilotonnes of CO2 equivalents in the grassland mineral soils change subsector in 2021 by source, colored by greenhouse gas identity."}
...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Bar graph showing greenhouse gas emissions in kilotonnes of CO2 equivalents in the forestry subsector in 2021 by source, colored by greenhouse gas identity."}
...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Bar graph showing greenhouse gas emissions in kilotonnes of CO2 equivalents in the peatland subsector in 2021 by source, colored by greenhouse gas identity."}
...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Bar graph showing greenhouse gas emissions in kilotonnes of CO2 equivalents in the settlement subsector in 2021 by source, colored by greenhouse gas identity."}
...
transport <- GHG_regions2 %>%
  filter(TES_Sector == "Domestic transport") %>% #filter sector
  filter(EmissionYear == "2021") %>% #filter to only 2021 
  group_by(Subsector, IPCC_name, GHG, IPCC_code) %>%
  summarize(Temissions = sum(Emission, na.rm = TRUE)) 
transport$IPCC_name <- gsub('_', ' ', transport$IPCC_name) #replace underscores with spaces to make nicer to read
...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Sankey diagram demonstrating breakdown of greenhouse gas emissions in the UK's transport sector in 2021. From left to right, greenhouse gas by subsector by individual source, following IPCC source names."}

#Set up Sankey links dataframe
transportlinks <- data.frame(source=c(paste0(transport$GHG), paste0(transport$Subsector)), 
                    target=c(paste0(transport$Subsector), paste0(transport$IPCC_name)),
