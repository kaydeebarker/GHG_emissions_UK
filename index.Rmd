...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Sunburst diagram demonstrating proportional breakdown of greenhouse gas emissions in the UK in 2022."}
...
cat_1 <- GHG_UK2022pos %>%
  ...
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  unite('parents', c(tes_sector, tes_subsector, category_2), remove = TRUE, sep = " - ") #combine higher categories into parents

activities <- GHG_UK2022pos %>%
  group_by(tes_sector, tes_subsector, category_2, category_1, activity) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  unite('parents', c(tes_sector, tes_subsector, category_2, category_1), remove = TRUE, sep = " - ") #combine higher categories into parents
...
GHGsb2 <- data.frame(
  parents =c(paste0(subsectors$tes_sector), paste0(cat_2$parents), paste0(cat_1$parents), paste0(activities$parents)),
  labels =c(paste0(subsectors$tes_subsector), paste0(cat_2$category_2), paste0(cat_1$category_1), paste0(activities$activity)),
  values =as.numeric(c(paste0(subsectors$Temissions), paste0(cat_2$Temissions), paste0(cat_1$Temissions), paste0(activities$Temissions)))
)
...
```{r, warning=FALSE, fig.align = "center", out.width = '95%', fig.cap="Sankey diagram demonstrating proportional breakdown of greenhouse gas emissions in the UK in 2022. From left to right, greenhouse gas by sector by subsector. Proportions based on megatonnesof CO2 equivalents."}
