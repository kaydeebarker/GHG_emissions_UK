...
circlepackeR(GHGtree2022, size = "emissions_mt_co2e", color_min = "hsl(56,80%,80%)", color_max = "hsl(341,30%,40%)")
...
GHGsb <- GHG_UK2022 %>%
          filter(tes_sector != "LULUCF") %>% #filter out LULUCF
          group_by(tes_sector, tes_subsector) %>%
          summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
          unite('ids', c(tes_sector, tes_subsector), remove = FALSE, sep = "-") #combine columns for id
...
sb <- sb %>% add_trace(
  type = 'sunburst',
  ids = GHGsb$ids,
  labels = GHGsb$tes_subsector,
  parents = GHGsb$tes_sector,
  values = GHGsb$Temissions,
  branchvalues = 'total',
