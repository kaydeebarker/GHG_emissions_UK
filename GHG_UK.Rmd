...
library(plotly) #interactive web graphs (install with Github)
library(networkD3) #interactive web network graphs (install with Github)
...
```{r sankey networkD3, warning=FALSE}
...
```{r sankey flipplot, warning=FALSE}

sankey2 <- SankeyDiagram(
  data = links,
  output.data.only = FALSE,
  weights = links$value,
  font.size = 12,
  font.family = "Times New Roman",
  font.unit = "pt", #sets font size to point instead of pixels
  colors = NULL,
  link.color = "Source", #sets link color to same as source node
  variables.share.values = FALSE,
  label.show.varname = TRUE, #show the variable name in the node label
  label.max.length = 100, #maximum characters in label
  label.show.counts = FALSE, #show count data in label
  label.show.percentages = FALSE, #show percent in label
  node.position.automatic = TRUE,
  node.width = 30,
  node.padding = 10, #vertical space between nodes
  sinks.right = TRUE, 
  hovertext.show.percentages = TRUE #show percent in hover label
)
...
```{r data seubsets, warning=FALSE}

ag_df <- GHG_regions2 %>%
  filter(TES_Sector == "Agriculture") %>% #filter sector
  filter(EmissionYear == "2021") %>% #filter to only 2021 
  group_by(Subsector, IPCC_name, GHG) %>%
  summarize(Temissions = sum(Emission, na.rm = TRUE))
